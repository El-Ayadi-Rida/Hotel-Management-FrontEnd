...
import { fRoomStatus } from './ChartSlice';

const RoomStatus = () => {
  const dispatch = useDispatch();
  const { themeValues } = useSelector((state) => state.settings);
  const { roomStatus , roomStatusStatus , roomStatusError } = useSelector((state) => state.chart);
  const chartContainer = useRef(null);


    // 📥 Fetch Room Status from backend
    useEffect(() => {
        dispatch(fRoomStatus());
    }, [dispatch]);
  ...
  const data = React.useMemo(() => {
    return {
      labels: roomStatus?.map(d => d?.status),
      datasets: [
        {
          label: '',
